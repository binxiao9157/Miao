...
export default function UploadMaterial() {
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      // 保留原本猫窝客厅场景控制，同时只提取并保持猫咪身份特征
      const prompt = `基于输入猫咪照片，将猫咪主体取出并重新生成一张照片。只保留输入照片中猫咪本身的身份特征，不保留输入照片的原始背景、原始构图、墙体、门框、窗户、栏杆、家具位置或探头姿态。

主体要求：输入照片中的猫咪必须作为唯一身份参考。精确提取并保持原图猫咪的全部外观特征，包括毛色、毛发花纹走向、体型比例、脸型、耳朵形状、鼻口区域、眼睛颜色、毛发长度和整体气质，保持绝对不变。不得替换为另一只猫，不得生成同品种或相似花色的泛化猫咪，不得随机生成猫咪。

姿态要求：在不改变猫咪身份特征的前提下，将猫咪轻微整理为标准蹲坐姿态，前爪并拢，身体端正，头部微微抬起，双眼圆睁，目光平静且专注地直视镜头。姿态可以调整，但脸部细节、五官比例、毛色、花纹、眼睛颜色、耳朵形状和体型比例不得被重构或扭曲。

场景环境：必须移除输入照片中的原始背景，只将猫咪主体重新放置在一张带有简约几何暗纹的柔软米色猫窝中，猫咪和猫窝整体位于画面正中央。背景必须是温馨的现代客厅，不要出现原图中的墙体、门框、窗户、栏杆、室外环境或杂乱背景。

光影与参数：全身照视角，中心构图，竖屏 9:16，480P。镜头平视视角（与猫眼齐平），浅景深虚化背景（f/2.8光圈效果）以突出猫咪。色温3500K的柔和室内自然光，营造治愈宁静的氛围。照片级超写实画质，毛发纹理极度清晰。画面四周保留适度空白，确保猫咪头顶、耳朵、全身、脚爪与猫窝底座均完整入画，不裁切。`.trim();
      
      const negativePrompt = `改变猫咪毛色，改变猫咪花纹，不同毛皮花纹走向，不同眼睛颜色，改变猫咪品种，改变脸型，改变耳朵形状，替换为另一只猫，同品种泛化，随机生成猫咪，保留原图背景，原图墙体，原图门框，原图窗户，原图栏杆，室外环境，杂乱背景，探头姿态，侧身躲藏姿态，非猫窝场景，特写镜头，大头照，主体充满画面，画面裁切，截断的耳朵，截断的爪子，卡通化，绘画感，艺术化处理，添加文字，水印，签名，多余的玩具，食物，其他人物，其他动物，画面畸变。`.trim();
