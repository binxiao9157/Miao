...
export default function UploadMaterial() {
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      // 使用全新重构的、保证主体一致性的首要优先级提示词
      const prompt = `最高优先级：输入照片中的猫咪必须作为唯一身份参考。必须百分之百保留原图猫咪的毛色、毛发花纹走向、脸型、耳朵形状、鼻口区域、眼睛颜色、毛发长度、体型比例和整体气质。不得将其替换为其他猫，绝对不准只生成同品种或相似花色的泛化猫咪。画面场景和构图可以重构，但猫咪身份必须与输入照片保持绝对一致。

基于上述极严格的身份约束，生成一张照片级真实的猫咪数字替身形象：
1. 【姿态与情绪】：猫咪可以被轻微整理为端正的蹲坐姿态，目光平静直视镜头，前爪自然靠近。但是脸部细节、五官比例绝对不得因姿态改变而被重构或扭曲。
2. 【场景环境】：猫咪舒服地蹲坐在一张带简约几何暗纹的温暖米色猫窝正中。背景是温馨怡人的现代客厅，配以色温 3500K 的柔和室内自然光。
3. 【构图与参数】：竖屏 9:16。使用平视视角的镜头拍摄，浅景深虚化背景（突出主体）。照片级超写实，毛发纹理根根分明，高保真度。画面四周保留适度空白，确保猫咪头顶、耳朵、全身、脚爪与猫窝底座均清晰完整入画，不进行任何阶段性截断或裁切。`.trim();
      
      const negativePrompt = `改变猫咪毛色，改变猫咪花纹，不同毛皮花纹走向，不同眼睛颜色，改变猫咪品种，改变脸型，改变耳朵形状，替换为另一只猫，同品种泛化，随机生成猫咪，特写镜头，大头照，主体充满画面，画面裁切，截断的耳朵，截断的爪子，卡通化，绘画感，艺术化处理，添加文字，水印，签名，多余的玩具，食物，其他人物，其他动物，画面畸变。`.trim();
